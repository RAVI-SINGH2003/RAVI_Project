/**
 * @fileoverview return font/char name for given css-class/char-name
 *
 * @author himanshu.garg@cse.iitd.ac.in
 */

export class FontInfo {
  public static path = '/fonts/font_info.json';
  ...
  public getCharName(cssClassName: string, c: string) {
    let charCode = c.charCodeAt(0);
    // Likely a private unicode mapping by pdf2htmlEX
    if (charCode > 0xe600) {
      charCode -= 0xe600;
    }

    // If we know its name from the font table
    if (this.json[cssClassName].CharNames[charCode]) {
      return this.json[cssClassName].CharNames[charCode];
    }

    // If this was converted to unicode from its name then unconvert
    // eslint-disable-next-line no-ternary
    return this.nameFor.has(charCode) ? this.nameFor.get(charCode) : c;
  }

  public getFontName(cssClassName: string) {
    if (cssClassName in this.json) {
      return this.json[cssClassName].FontName;
    }

    return this.json.f1.FontName;
  }

  // Map character names to Unicode
  nameFor = new Map([
    /*
     * Extracted from poppler/src/NameToUnicodeTable.h
     * =====================================================================
     *
     ...
     *
     * To see a description of the changes please see the
     * Changelog file that
     * Came with your tarball or type make ChangeLog
